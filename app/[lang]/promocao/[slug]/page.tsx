...
// Convierte los trechos marcados con **...** (ej. o cupom **CORRIDA26**) en negrita.
function withBold(text: string) {
  return text.split(/\*\*(.+?)\*\*/g).map((part, i) =>
    i % 2 === 1 ? (
      <strong key={i} style={{ fontWeight: 700, color: "#143C7A" }}>
        {part}
      </strong>
    ) : (
      part
    ...
    <>
      ...
      <section className="pp-prod">
        ...
        <div>
          ...
          <p style={{ fontSize: "15px", lineHeight: 1.8, color: "#5C6B7A", margin: "26px 0 0" }}>{withBold(p.desc1)}</p>
          <p style={{ fontSize: "15px", lineHeight: 1.8, color: "#5C6B7A", margin: "16px 0 0" }}>{withBold(p.desc2)}</p>

          <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginTop: "36px" }}>
            <a href={p.waMessage ? whatsappHref(p.waMessage) : WHATSAPP_HREF} target="_blank" rel="noopener noreferrer" className="pp-wa" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", height: "54px", background: "#143C7A", color: "#FFFFFF", textDecoration: "none", fontSize: "12px", letterSpacing: ".22em", textTransform: "uppercase", transition: "filter .15s ease" }}>
            ...
            <a href={buildOfferBookingUrl(p.booking)} target="_blank" rel="noopener noreferrer" className="pp-resv" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "54px", background: "#C79A6A", color: "#FFFFFF", textDecoration: "none", fontSize: "12px", letterSpacing: ".22em", textTransform: "uppercase", transition: "filter .15s ease" }}>{dict.promocaoProduct.reservarAgora}</a>
          </div>
        </div>
      </section>
